...
from mani_skill.envs.tasks import TwoRobotStack3Cube
...
def solve(env:TwoRobotStack3Cube, seed, debug, vis):
    env.reset(seed=seed)
    ...
    grasp_C_approach_pose = grasp_C_pose*sapien.Pose(p=[0,0,-0.1])
    ...
    # Reset A
    reset_A_pose = sapien.Pose(p=[-0.4,-0.254,1.018],q=[0.138,0.694,0.694,-0.138])
    result = planner.move_to_pose_with_screw(
        reset_A_pose,
        arm_index=1
    )
    ...
    # Reset B
    reset_B_pose = sapien.Pose(p=[-0.4,0.254,1.018],q=[0.138,-0.694,0.694,0.138])
    result = planner.move_to_pose_with_screw(
        reset_B_pose,
        arm_index=2
    )
    ...
    # Place C
    place_C_pose = place_C_pose*sapien.Pose(p=[0,0,-0.1])
    result = planner.move_to_pose_with_screw(
        place_C_pose,
        arm_index=1
    )
