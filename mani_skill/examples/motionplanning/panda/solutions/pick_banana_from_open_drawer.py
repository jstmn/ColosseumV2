...
from mani_skill.envs.tasks.tabletop.colosseum_v2.pick_banana_from_open_drawer import PickBananaFromOpenDrawerEnv
from mani_skill.examples.motionplanning.panda.motionplanner import PandaArmMotionPlanningSolver
from mani_skill.examples.motionplanning.base_motionplanner.utils import compute_grasp_info_by_obb, get_actor_obb


def main():
    env: PickBananaFromOpenDrawerEnv = gym.make(
...
def solve(env: PickBananaFromOpenDrawerEnv, seed=None, debug=False, vis=False):
    ...
    lift_pose = sapien.Pose(
        p=[grasp_pose.p[0], grasp_pose.p[1], env.LIFT_HEIGHT + 0.05],
        q=grasp_pose.q
    )
    ...
    retract_pose = lift_pose * sapien.Pose([0, 0, -0.1])
