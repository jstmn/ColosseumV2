...
def convert():
    data_dir = "new_demos_all/hyeonho_feb17_all_cameras/lift_peg_50_4camera"
    output_dir = "new_demos_all/real_data/processed/eih/lift_peg_50"
    ...
    # 1. 카메라 정보 로드
    cam_info_path = os.path.join(data_dir, "eih_camera_info_color.json")
    ...
    with h5py.File(output_h5, "w") as out_f:
        for i in tqdm(range(50), desc="Converting to 128x128"):
            ...
            with h5py.File(h5_name, "r") as in_f:
                ...
                # 원본 이미지 로드
                raw_images = in_f["eih__color_image"][:]
                ...
                # 3. 리사이징된 이미지 저장
                sensor_data = obs.create_group("sensor_data/eih_camera")
                sensor_data.create_dataset("rgb", data=resized_images[:-1])
                
                # 4. 수정된 Intrinsic 적용
                sensor_param = obs.create_group("sensor_param/eih_camera")
